<i18n lang="json5">
{
    en: {
    '1': 'THANK YOU',
    '2': 'Thank you for confirming your email.',
    '3': 'Go Back to Home',
    },
    ar: {
        '1': 'شكرًا لك',
        '2': 'شكرا لتأكيد بريدك الإلكتروني',
        '3': 'ارجع إلى الصفحة الرئيسية',
    },
}
</i18n>
<template>
    <div class="w100 d-flex flex-column align-center justify-center thankyou">
        <div class="erxa-logo w100 text-center">
            <img src="https://erxa-files.s3.amazonaws.com/assets/erxa_one.png" alt="">
        </div>
        <div class="thankyou-text">
            <h3 class="text-h5 text-md-h4 font-weight-bold">{{ $t('1') }}</h3>
            <p class="text-caption text-md-h6">{{ $t('2') }}</p>
        </div>
        <v-row>
            <div class="download-app-btns">
                <v-btn
                    large
                    depressed
                    color="#b42c2c"
                    class="ma-5 white--text"
                    href="https://apps.apple.com/in/app/erxa-expert/id1576109528"
                >
                    <v-icon>{{ mdiApple }}</v-icon>
                    <p><span>Download on the</span><br>App Store</p>
                </v-btn>
                <v-btn
                    large
                    depressed
                    color="#30bc8f"
                    class="ma-5 white--text"
                    href="https://erxa-files.s3.amazonaws.com/erxa-expert.apk"
                    ><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="36px" height="36px" style="shape-rendering:geometricPrecision; text-rendering:geometricPrecision; image-rendering:optimizeQuality; fill-rule:evenodd; clip-rule:evenodd">
                    <g><path style="opacity:0.852" fill="#7bb242" d="M 11.5,3.5 C 13.4649,4.129 15.4649,4.79566 17.5,5.5C 19.1349,5.12229 20.6349,4.45563 22,3.5C 22.5389,5.71296 23.7055,8.0463 25.5,10.5C 20.1667,11.8333 14.8333,11.8333 9.5,10.5C 10.1279,8.74186 11.1279,7.24186 12.5,6C 11.7476,5.32924 11.4142,4.4959 11.5,3.5 Z"/></g>
                    <g><path style="opacity:0.924" fill="#7cb242" d="M 5.5,12.5 C 6.5,12.5 7.5,12.5 8.5,12.5C 8.5,16.1667 8.5,19.8333 8.5,23.5C 7.5,23.5 6.5,23.5 5.5,23.5C 5.5,19.8333 5.5,16.1667 5.5,12.5 Z"/></g>
                    <g><path style="opacity:0.854" fill="#7bb441" d="M 9.5,12.5 C 14.8333,12.5 20.1667,12.5 25.5,12.5C 25.6657,16.8461 25.499,21.1794 25,25.5C 22.9838,26.9552 22.1505,28.9552 22.5,31.5C 21.1667,31.5 19.8333,31.5 18.5,31.5C 17.8333,24.8333 17.1667,24.8333 16.5,31.5C 15.1667,31.5 13.8333,31.5 12.5,31.5C 12.8495,28.9552 12.0162,26.9552 10,25.5C 9.50098,21.1794 9.33432,16.8461 9.5,12.5 Z"/></g>
                    <g><path style="opacity:0.924" fill="#7cb242" d="M 26.5,12.5 C 27.5,12.5 28.5,12.5 29.5,12.5C 29.5,16.1667 29.5,19.8333 29.5,23.5C 28.5,23.5 27.5,23.5 26.5,23.5C 26.5,19.8333 26.5,16.1667 26.5,12.5 Z"/></g>
                    </svg>
                    <p><span>Download</span><br>Android App</p>
                </v-btn>
            </div>
        </v-row>
        <v-row>
            <v-btn class="go-back-btn">
                <nuxt-link :to="localePath('/')">
                    {{ $t('3') }}
                </nuxt-link>
            </v-btn>
        </v-row>
    </div>
</template>

<script>
  import { mdiApple } from '@mdi/js';
export default {
    components: {},
    data: () => ({ mdiApple }),
};
</script>

<style scoped>
.thankyou {
    width: 100%;
    max-width: 500px;
    box-shadow: 0 0.1429rem 0.6429rem 0.07143rem rgb(1 2 2 / 10%);
    padding: 50px 20px;
    background: #fff;
    border-radius: 10px;
    box-shadow: rgb(0 0 0 / 35%) 0px 5px 15px;
}
.thankyou .erxa-logo {
    width: 165px;
}
.thankyou img {
    width: 100%;
    height: auto;
}
.thankyou .thankyou-text {
    padding: 20px 30px;
    text-align: center;
}
.thankyou .thankyou-text h3 {
    color: #2fba8f;
}
.thankyou .thankyou-text p {
    color: #938f8f;
}
.erxa-logo > svg {
    width: 20%;
    height: 30%;
}
.erxa-logo > svg > g {
    fill: var(--primary-color);
}
.thankyou .go-back-btn {
    height: 40px;
    width: 165px;
    background: transparent;
    border-radius: 20px;
    box-shadow: unset;
    border: 1px solid #2fba8f;
    outline: none;
}
.thankyou .go-back-btn:hover {
    background-color: #e7f7f2;
}
.thankyou .go-back-btn a {
    text-decoration: none;
    text-transform: capitalize;
    outline: none;
}
.download-app-btns {
    padding-bottom: 20px;
}
.download-app-btns .v-btn {
    background-color: transparent !important;
    box-shadow: 0px 3px 1px -2px rgb(0 0 0 / 20%), 0px 2px 2px 0px rgb(0 0 0 / 14%), 0px 1px 5px 0px rgb(0 0 0 / 12%);
    color: #000 !important;
  }
.download-app-btns .v-btn p {
    margin: 0;
    color: #000;
    font-weight: 700;
    font-size: 12px;
    margin-left: 5px;
  }
.download-app-btns .v-btn p span {
    font-weight: 100;
    color: #8d8a8a;
  }
.download-app-btns .v-btn .v-icon svg {
    width: 34px;
    height: 34px;
  }
  @media (max-width: 436px) {
    .download-app-btns {
        text-align: center;
    }
    .download-app-btns a {
        width: 170px;
    }
  }
</style>
